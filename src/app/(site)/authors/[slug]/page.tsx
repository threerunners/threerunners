...
import ashleyData from '@/data/authors/ashley-morgan.json'
import type { Author } from '@/types/author'
import BreadcrumbNav from '@/components/shared/BreadcrumbNav'
import Link from 'next/link'

const authors: Author[] = [ashleyData as Author]
...
export function generateMetadata({ params }: PageProps): Metadata {
  ...
  if (!author) return {}
  return {
    title: `${author.name} | The Run Down`,
    description: author.voiceLine,
  }
}
    ...
    <div className="wrap-narrow" style={{ paddingBottom: 72 }}>
      ...
      <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', marginBottom: 40, flexWrap: 'wrap' }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 15, color: 'var(--ink-600)', lineHeight: 1.6, margin: '0', maxWidth: '56ch' }}>
            {author.voiceLine}
          </p>
        </div>
      </div>
      ...
      <div style={{
        ...
        background: 'var(--ink-50)', borderRadius: 'var(--radius-lg)', padding: '20px 24px',
      }}>
        {[
          ...
          <div key={stat.label}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--ink-500)', marginBottom: 4 }}>
              {stat.label}
            </div>
      ...
      <div style={{ marginBottom: 48 }}>
        {paragraphs.map((para, idx) => (
          <p key={idx} style={{
            fontSize: 15.5, color: 'var(--ink-700)', lineHeight: 1.75,
            margin: idx < paragraphs.length - 1 ? '0 0 18px' : 0,
            maxWidth: '62ch',
          }}>
            {para}
          </p>
      ...
      {author.authoredSpokes.length > 0 && (
        <div>
          <h2 style={{
            ...
            letterSpacing: '-0.01em', margin: '0 0 16px', color: 'var(--ink-900)',
          }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {author.authoredSpokes.map(spokeId => (
              <Link
                key={spokeId}
                href={`/marathon/${spokeId.replace('marathon-', '')}`}
                style={{
              ...
              >
                ...
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ width: 14, height: 14, color: 'var(--ink-500)' }}>
